"""
Train or evaluate a ConStruct model from the TopoGen repo root.

This wrapper mirrors ConStruct/main.py but:
  - Uses TopoGen's Hydra config tree (configs/) so datadir resolves to TopoGen/data/
  - Imports DataModules and model classes directly from baselines/ConStruct
  - Writes checkpoints and outputs under TopoGen/outputs/

Usage
-----
Train:
    python train_construct.py model=construct dataset=planar general.name=construct_planar_absorbing_edges

Train with a specific transition variant:
    python train_construct.py model=construct dataset=qm9    general.name=construct_qm9_uniform    model.transition=uniform
    python train_construct.py model=construct dataset=qm9    general.name=construct_qm9_marginal   model.transition=marginal
    python train_construct.py model=construct dataset=qm9    general.name=construct_qm9_absorbing  model.transition=absorbing
    python train_construct.py model=construct dataset=qm9    general.name=construct_qm9_abs_edges  model.transition=absorbing_edges

Evaluate only (runs 5 seeds):
    python train_construct.py model=construct dataset=planar general.name=eval_planar \
        general.test_only=/abs/path/to/checkpoint.ckpt

Use a preset experiment config (overrides all defaults):
    python train_construct.py +construct_experiment=planar
"""
...
try:
    ...
    print("[train_construct] graph_tool not found — planarity/isomorphism metrics unavailable")
...
def _resolve_datadir(cfg: DictConfig) -> str:
    """Return absolute path to the dataset root, resolving relative datadir
    against TopoGen/data/ (not Hydra's output dir)."""
...
def _build_datamodule(cfg: DictConfig, root_path: str):
    """Instantiate the correct DataModule and DatasetInfos for cfg.dataset.name."""
    name = cfg.dataset.name

    # Patch cfg so DataModules that call get_original_cwd() still work:
    # we inject the absolute root_path directly via a cfg override rather than
    # relying on the get_original_cwd() + parents[0] + datadir chain.
...
def _dump_metrics(metrics: dict, path: str) -> None:
    """Write trainer.logged_metrics to a JSON file, skipping non-scalar values."""
...
def _placeholder_batch_to_nx_list(batch):
    ...
    import numpy as np
    X = batch.X.detach().cpu().numpy()              # (B, n_max)
    E = batch.E.detach().cpu().numpy()              # (B, n_max, n_max) or (B, n_max, n_max, K)
    mask = batch.node_mask.detach().cpu().numpy()   # (B, n_max), bool
    if E.ndim == 4:
        adj = (E.argmax(-1) > 0).astype(np.int8)    # treat any non-zero edge type as edge
    else:
...
def _dump_generated_graphs(samples_pkl_path: str, out_path: str):
    """Read generated_samples_rank0.pkl (PlaceHolder list) and write a nx.Graph list pickle."""
...
def _dump_reference_graphs(datamodule, split: str, out_path: str):
    """Convert datamodule's val/test set to a list of nx.Graph objects and pickle."""
    import pickle as _pkl
    ...
    for batch in loader_fn():
        for data in batch.to_data_list():
            graphs.append(
                to_networkx(data, node_attrs=None, edge_attrs=None,
...
def main(cfg: DictConfig) -> None:
    ...
    print(f"[train_construct] dataset  : {cfg.dataset.name}")
    print(f"[train_construct] data root: {root_path}")
    print(f"[train_construct] run name : {cfg.general.name}")
    print(f"[train_construct] transition: {cfg.model.transition}")
    print(f"[train_construct] rev_proj  : {cfg.model.rev_proj}")

    # Must import before _build_datamodule: SpectreDatasetInfos calls compute_reference_metrics
    # which does `metrics.sampling_metrics.SamplingMetrics` — the submodule must be in sys.modules first.
    ...
    # DDP: ignore non-serialisable metric objects
    params_to_ignore = [
    ...
    if not cfg.general.test_only:
        ...
        print(f"[train_construct] training time: {fit_time_s:.1f}s ({fit_time_s/3600:.2f}h)")
        if cfg.train.save_model:
            print(f"[train_construct] best ckpt: {checkpoint_callback.best_model_path}")
        _dump_metrics({"train/total_time_s": fit_time_s}, "train_timing.json")
        # Test is run separately via general.test_only=<ckpt_path>
        # trainer.test(model, datamodule=datamodule,
        #              ckpt_path=checkpoint_callback.best_model_path if cfg.train.save_model else None)
    else:
        # ── Step 0: pick top-3 NLL ckpts via best_k_models embedded in any ckpt ─
        # Lightning's ModelCheckpoint(monitor='val/epoch_NLL', save_top_k=5) records
        # per-ckpt val NLL inside every saved file under callbacks[*].best_k_models.
        # The latest-epoch ckpt has the most up-to-date dict.
        ckpt_dir = pathlib.Path(cfg.general.test_only).parent

        def _epoch_of(p):
            import re
            m = re.search(r"epoch=(\d+)", pathlib.Path(p).name)
        ...
        # bkm: {original_path_str: nll_tensor}; resolve to current ckpt_dir by basename
        nll_pairs = []
        ...
        top3 = nll_pairs[:3]
        print(f"[train_construct] best_k_models has {len(nll_pairs)} resolvable ckpts; selecting top-3 by NLL:")
        for p, nll in top3:
            print(f"  NLL={nll:.4f}  {p.name}")

        # ── Match samples_to_generate to dataset split sizes ────────────────
        ...
        print(f"[train_construct] samples_to_generate: val={n_val}  test={n_test}")

        # ── Step 1: validate each top-3 ckpt; rank by mean of raw MMDs ──────
        ...
        for ckpt_file, nll in top3:
            print(f"[train_construct] val-scanning: {ckpt_file.name}  (NLL={nll:.4f})")
            pl.seed_everything(cfg.train.seed)
            ...
            print(f"[train_construct]   mmd_avg = {mmd_avg:.4f}  "
                  f"(degree={metrics.get('degree','-')}, clustering={metrics.get('clustering','-')}, "
                  f"orbit={metrics.get('orbit','-')}, spectre={metrics.get('spectre','-')}, "
                  f"wavelet={metrics.get('wavelet','-')})")
            if mmd_avg < best_score:
                best_score = mmd_avg
                best_ckpt_path = str(ckpt_file)

        # Persist the top-3 summary
        _dump_metrics(
        ...
        print(f"\n[train_construct] top-3 ranking by mean MMD (lower = better):")
        ...
        # ── Step 2: 5-seed final eval on best ckpt, on test set ─────────────
        print(f"\n[train_construct] running 5-seed test eval on: {best_ckpt_path}")
        seed_metric_paths = []
        seed_graph_lists = {}   # seed -> list[nx.Graph]
        for seed_idx in range(5):
            new_seed = seed_idx * 1000
            pl.seed_everything(new_seed)
            with open_dict_safe(cfg) as c:
                c.train.seed = new_seed
            print(f"[train_construct] seed {new_seed}")
            ...
            # Convert this seed's PlaceHolder samples to nx.Graph list
        ...
        # Combined per-seed dump (dict[seed -> list[nx.Graph]]) + reference test graphs
        if seed_graph_lists:
            import pickle as _pkl
            with open("test/test_graphs.pkl", "wb") as f:
                _pkl.dump(seed_graph_lists, f, protocol=_pkl.HIGHEST_PROTOCOL)
            print(f"[train_construct] dumped test/test_graphs.pkl: dict[seed → list[nx.Graph]] "
                  f"({len(seed_graph_lists)} seeds × ~{len(next(iter(seed_graph_lists.values())))} graphs)")
        try:
            ref_graphs = _dump_reference_graphs(datamodule, "test", "test/reference_graphs.pkl")
            print(f"[train_construct] dumped test/reference_graphs.pkl: {len(ref_graphs)} nx.Graph reference graphs")
        except Exception as e:
            print(f"[train_construct] WARNING: could not dump reference graphs: {e}")

        # ── Step 3: aggregate test metrics across 5 seeds (mean + std) ──────
        import statistics
        per_seed = []
        for p in seed_metric_paths:
            try:
                ...
                print(f"[train_construct] WARNING: could not read {p}: {e}")
        if per_seed:
            ...
            print(f"\n[train_construct] 5-seed aggregate written to test/seed_aggregate.json")
            print(f"[train_construct] per-graph generation time: mean={tg_mean:.3f}s std={tg_std:.3f}s (over {agg.get('test/per_graph_generation_time_s/n', 0)} seeds, {n_test} graphs each)")
