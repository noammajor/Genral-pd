"""
Train or evaluate a DiGress-equivalent model (ConStruct backbone, marginal transition).

Uses the same ConStruct model and datamodule as train_construct.py, but with
transition=marginal and rev_proj=False — matching DiGress's training setup.

Usage
-----
Train:
    python train_digress.py +digress_experiment=planar general.name=planar_digress_full

Evaluate only (runs 5 seeds):
    python train_digress.py +digress_experiment=planar general.name=eval_planar \
...
try:
    ...
    print("[train_digress] graph_tool not found — planarity/isomorphism metrics unavailable")
...
@hydra.main(version_base="1.3", config_path="configs", config_name="config_digress")
def main(cfg: DictConfig) -> None:
    pl.seed_everything(cfg.train.seed)

    root_path = _resolve_datadir(cfg)
    print(f"[train_digress] dataset   : {cfg.dataset.name}")
    print(f"[train_digress] data root : {root_path}")
    print(f"[train_digress] run name  : {cfg.general.name}")
    print(f"[train_digress] transition: {cfg.model.transition}")
    ...
    if not cfg.general.test_only:
        ...
        print(f"[train_digress] training time: {fit_time_s:.1f}s ({fit_time_s/3600:.2f}h)")
        if cfg.train.save_model:
            print(f"[train_digress] best ckpt: {checkpoint_callback.best_model_path}")
        ...
        top3 = nll_pairs[:3]
        print(f"[train_digress] best_k_models has {len(nll_pairs)} resolvable ckpts; selecting top-3 by NLL:")
        for p, nll in top3:
        ...
        print(f"[train_digress] samples_to_generate: val={n_val}  test={n_test}")
        ...
        for ckpt_file, nll in top3:
            print(f"[train_digress] val-scanning: {ckpt_file.name}  (NLL={nll:.4f})")
            pl.seed_everything(cfg.train.seed)
            ...
            print(f"[train_digress]   mmd_avg = {mmd_avg:.4f}")
        ...
        print(f"\n[train_digress] top-3 ranking by mean MMD (lower = better):")
        ...
        print(f"\n[train_digress] running 5-seed test eval on: {best_ckpt_path}")
        seed_metric_paths = []
        seed_graph_lists = {}
        for seed_idx in range(5):
            ...
            print(f"[train_digress] seed {new_seed}")
        ...
        if seed_graph_lists:
            ...
            print(f"[train_digress] dumped test/test_graphs.pkl: {len(seed_graph_lists)} seeds")
        try:
            ref_graphs = _dump_reference_graphs(datamodule, "test", "test/reference_graphs.pkl")
            print(f"[train_digress] dumped test/reference_graphs.pkl: {len(ref_graphs)} reference graphs")
        except Exception as e:
            print(f"[train_digress] WARNING: could not dump reference graphs: {e}")
        ...
        for p in seed_metric_paths:
            try:
                ...
                print(f"[train_digress] WARNING: could not read {p}: {e}")
        if per_seed:
            ...
            print(f"\n[train_digress] 5-seed aggregate written to test/seed_aggregate.json")
            print(f"[train_digress] per-graph generation time: mean={tg_mean:.3f}s std={tg_std:.3f}s")
