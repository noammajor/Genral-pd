...
class SpectreGraphDataset(InMemoryDataset):
    def __init__(
        ...
        self.mu_transform = mu_transform  # MuAttachTransform or None
    ...
    def get(self, idx):
        data = super().get(idx)
        if self.mu_transform is not None:
            self.mu_transform.set_idx(idx)
            data = self.mu_transform(data)
        return data
...
class SpectreGraphDataModule(AbstractDataModule):
    def __init__(self, cfg):
        ...
        # TAM: build per-split MuAttachTransforms if use_tam is enabled
        use_tam = getattr(cfg.model, "use_tam", False)
        mu_transforms = {split: None for split in ("train", "val", "test")}
        if use_tam:
            processed_dir = os.path.join(root_path, "processed")
            for split in ("train", "val", "test"):
                mu_path = os.path.join(processed_dir, f"mu_{split}.npy")
                if os.path.exists(mu_path):
                    mu_transforms[split] = MuAttachTransform(np.load(mu_path))
                    print(f"[TAM] Loaded {mu_path}")
                else:
                    print(f"[TAM] WARNING: {mu_path} not found. Run scripts/precompute_mu.py first.")
        ...
        datasets = {
            "train": SpectreGraphDataset(
                ...
                mu_transform=mu_transforms["train"],
            ),
            "val": SpectreGraphDataset(
                ...
                split="val",
                root=root_path,
                mu_transform=mu_transforms["val"],
            ),
            "test": SpectreGraphDataset(
                ...
                split="test",
                root=root_path,
                mu_transform=mu_transforms["test"],
            ),
        }
...
class SpectreDatasetInfos(AbstractDatasetInfos):
    def __init__(self, datamodule, cfg=None):
        ...
        training_mode = cfg is not None and not cfg.general.test_only
        compute_reference_metrics(self, datamodule, training_mode=training_mode)
