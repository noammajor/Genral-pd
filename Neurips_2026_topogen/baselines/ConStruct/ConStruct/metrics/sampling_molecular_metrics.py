...
class SamplingMolecularMetrics(nn.Module):
    ...
    def forward(self, generated_graphs: list[PlaceHolder], current_epoch, local_rank, save_dir: str = "."):
        molecules = []
        ...
        # Save SMILES into save_dir
        os.makedirs(save_dir, exist_ok=True)
        smiles_path = os.path.join(save_dir, f"valid_unique_molecules_e{current_epoch}_GR{local_rank}.txt")
        with open(smiles_path, "w") as textfile:
            textfile.writelines(all_generated_smiles)
        if self.test:
            final_path = os.path.join(save_dir, f"final_smiles_GR{local_rank}.txt")
