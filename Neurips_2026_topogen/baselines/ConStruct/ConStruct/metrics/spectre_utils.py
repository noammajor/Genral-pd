...
class SpectreSamplingMetrics(nn.Module):
    ...
    def forward(
        ...
        np.savez(os.path.join(save_dir, "generated_adjs.npz"), *adjacency_matrices)
...
class SBMSamplingMetrics(SpectreSamplingMetrics):
    def __init__(self, train_dataloader, val_dataloader):
        # NOTE: dropped "sbm" from metrics_list. The "sbm" entry triggers
        # eval_acc_sbm_graph, which runs graph_tool's MCMC blockmodel
        # inference in a ThreadPoolExecutor. graph_tool is not thread-safe
        # for that workload and reliably crashes the Python process with
        # `double free or corruption (!prev)` on sbm-sized graphs (n up to 174).
        # We don't need the sbm_acc metric for our paper, and the 5 SPECTRE
        # MMDs (degree, clustering, orbit, spectre, wavelet) are unaffected.
        super().__init__(
            train_dataloader=train_dataloader,
            val_dataloader=val_dataloader,
            compute_emd=False,
