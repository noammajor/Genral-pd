...
from topo_gen.pdm_utils import pdm_loss_from_adj


class _ProgressTracker:
    """Lightweight progress reporter for graph-sampling loops.

    Reports elapsed, ETA, and per-graph rate after each `update(n)` call.
    Prints with carriage return (in-place) when stream is a TTY, newline otherwise.
    """
    def __init__(self, total: int, label: str = "sampling", every_n: int = 1):
    ...
    def update(self, n: int = 1):
        self.done += n
        if self.done % self.every_n != 0 and self.done < self.total:
            return
        elapsed = time.time() - self.start
        rate = elapsed / max(self.done, 1)
        remaining = max(self.total - self.done, 0) * rate
        eta_str = (f"{remaining/3600:.1f}h" if remaining > 3600
                   else f"{remaining/60:.1f}m" if remaining > 60
                   else f"{remaining:.1f}s")
        print(f"[{self.label}] {self.done:>4}/{self.total}  "
              f"elapsed={elapsed:6.1f}s  ETA={eta_str:>6}  "
              f"({rate:.2f}s/graph)", flush=True)
...
class DiscreteDenoisingDiffusion(pl.LightningModule):
    ...
    def __init__(self, cfg, dataset_infos, val_sampling_metrics, test_sampling_metrics):
        ...
        use_tam = getattr(cfg.model, "use_tam", False)
        tam_mu_dim = getattr(cfg.model, "tam_mu_dim", 1000) if use_tam else 0
        ...
        # PDM loss: load per-graph µ vectors if enabled
        self.use_pdm = getattr(cfg.model, "use_pdm", False)
        self.alpha_2 = getattr(cfg.model, "alpha_2", 0.001)
        self.mu_train = None  # (N_train, mu_dim) numpy array, loaded lazily
        if self.use_pdm:
            data_dir = Path(cfg.dataset.datadir) / "processed"
            mu_path = data_dir / "mu_train.npy"
            if mu_path.exists():
                self.mu_train = np.load(mu_path)  # (N_train, mu_dim)
                print(f"[PDM] Loaded mu_train {self.mu_train.shape} from {mu_path}")
            else:
                print(f"[PDM] WARNING: mu_train.npy not found at {mu_path}. Run scripts/precompute_mu.py first.")

    def forward(self, z_t, mu=None):
        ...
        return self.model(model_input, mu=mu)
    ...
    def _pdm_loss_batch(self, dense_data, pred) -> torch.Tensor:
        """Compute mean PDM loss over the batch. Runs on CPU (PH is CPU-only)."""
        # true binary adj: edge class > 0 means edge exists
        E_true = dense_data.E.argmax(-1)          # (bs, n, n)
        E_pred = pred.E.argmax(-1)                # (bs, n, n)
        node_mask = dense_data.node_mask          # (bs, n)

        E_true_np = E_true.detach().cpu().numpy()
        E_pred_np = E_pred.detach().cpu().numpy()
        mask_np   = node_mask.detach().cpu().numpy().astype(bool)

        bs = E_true_np.shape[0]
        total = 0.0
        for b in range(bs):
            adj_true = (E_true_np[b] > 0).astype(np.float32)
            adj_pred = (E_pred_np[b] > 0).astype(np.float32)
            total += pdm_loss_from_adj(adj_true, adj_pred, mask_np[b])

        mean_loss = total / bs
        return torch.tensor(mean_loss, dtype=torch.float32, device=self.device)
    ...
    def training_step(self, data, i):
        ...
        mu = data.mu.squeeze(1).to(self.device) if hasattr(data, "mu") else None
        pred = self.forward(z_t, mu=mu)
        ...
        if self.use_pdm:
            pdm = self._pdm_loss_batch(dense_data, pred)
            loss = loss + self.alpha_2 * pdm
            if i % self.log_every_steps == 0:
                self.log("train/pdm_loss", pdm, batch_size=self.BS)
    ...
    def on_validation_epoch_end(self) -> None:
        ...
        self.val_counter += 1
        sample_every = self.cfg.general.sample_every_val
        # current_epoch > 0 skips the pre-training sanity val; not needed when sample_every=1 (eval mode)
        epoch_guard = self.current_epoch > 0 if sample_every > 1 else True
        if self.name == "debug" or (
            self.val_counter % sample_every == 0
            and epoch_guard
        ):
            ...
            # TAGG conditional sampling at val: one generated graph per val reference,
            # conditioned on its mu. Mirrors the test path. Gated by use_tam.
            use_tam = getattr(self.cfg.model, "use_tam", False)
            if use_tam and hasattr(self._trainer, "datamodule") and self._trainer.datamodule is not None:
                val_loader = self._trainer.datamodule.val_dataloader()
                total_val = len(val_loader.dataset)
                progress = _ProgressTracker(total=total_val, label=f"val[rank{self.local_rank}]")
                samples = []
                for batch_idx, data in enumerate(val_loader):
                    mu = data.mu.squeeze(1).to(self.device) if hasattr(data, "mu") else None
                    dense_data = utils.to_dense(data, self.dataset_infos)
                    n_nodes = dense_data.node_mask.sum(dim=-1).long().tolist()
                    samples.append(
                        self.sample_batch(
                            n_nodes=n_nodes,
                            batch_id=batch_idx,
                    ...
                    progress.update(len(n_nodes))
                self.print(f"Conditional val sampling done: {sum(s.X.shape[0] for s in samples)} graphs")
            else:
                samples = self.sample_n_graphs(
                    ...
                    test=False,
                    progress_label=f"val[rank{self.local_rank}]",
                )

            val_gen_time = time.time() - start
            n_generated = sum(s.X.shape[0] for s in samples)
            per_graph_s = (val_gen_time / n_generated) if n_generated > 0 else float("nan")
            print(
                f"Sampled {len(samples)} batches ({n_generated} graphs) on local rank {self.local_rank}. "
                f"Sampling took {val_gen_time:.2f} seconds ({per_graph_s:.3f} s/graph)\n"
            )
            self.log("val/generation_time_s", val_gen_time, sync_dist=True)
            self.log("val/n_generated", float(n_generated), sync_dist=True)
            self.log("val/per_graph_generation_time_s", per_graph_s, sync_dist=True)
            print(f"Computing sampling metrics on {self.local_rank}...")
            save_dir = getattr(self, "_current_save_dir", ".")
            val_sampling_to_log, _ = self.val_sampling_metrics.compute_all_metrics(
                generated_graphs=samples,
                current_epoch=self.current_epoch,
                local_rank=self.local_rank,
                save_dir=save_dir,
            )
            scalar_val_sampling = {k: float(v) for k, v in val_sampling_to_log.items() if isinstance(v, (int, float, torch.Tensor))}
            self.log_dict(scalar_val_sampling, sync_dist=True)
    ...
    def on_test_epoch_end(self) -> None:
        ...
        # TAGG conditional sampling: one generated graph per test graph, conditioned on its mu.
        # Falls back to unconditional sample_n_graphs when use_tam is False.
        use_tam = getattr(self.cfg.model, "use_tam", False)
        if use_tam and hasattr(self._trainer, "datamodule") and self._trainer.datamodule is not None:
            test_loader = self._trainer.datamodule.test_dataloader()
            total_test = len(test_loader.dataset)
            progress = _ProgressTracker(total=total_test, label=f"test[rank{self.local_rank}]")
            samples = []
            for batch_idx, data in enumerate(test_loader):
                ...
                progress.update(len(n_nodes))
            self.print(f"Conditional sampling done: {sum(s.X.shape[0] for s in samples)} graphs")
        else:
            to_sample = math.ceil(
            ...
            samples = self.sample_n_graphs(
                ...
                test=True,
                progress_label=f"test[rank{self.local_rank}]",
            )
        # Stop generation timer here — before MMD/metric computation —
        # so test_gen_time matches val_gen_time semantics (sampling only).
        test_gen_time = time.time() - start
        n_generated = sum(getattr(s, "X").shape[0] for s in samples)
        per_graph_s = (test_gen_time / n_generated) if n_generated > 0 else float("nan")
        print(f"Generated {n_generated} graphs in {test_gen_time:.2f}s ({per_graph_s:.3f} s/graph)")
        ...
        with open(os.path.join(save_dir, f"generated_samples_rank{self.local_rank}.pkl"), "wb") as f:
            pickle.dump(samples, f)

        print("Saving the generated graphs")
        # This line is used to sync between gpus
        self._trainer.strategy.barrier()
        filename = os.path.join(save_dir, f"generated_samples_rank{self.local_rank}.txt")
        ...
        for i in range(self._trainer.num_devices):
            with open(os.path.join(save_dir, f"generated_samples_rank{i}.pkl"), "rb") as f:
                samples.extend(pickle.load(f))

        sampling_to_log, _ = self.test_sampling_metrics.compute_all_metrics(
            generated_graphs=samples,
            current_epoch=self.current_epoch,
            local_rank=self.local_rank,
            save_dir=save_dir,
        )
        # Log scalar sampling metrics so trainer.logged_metrics exposes them to train_construct.py
        scalar_sampling = {k: float(v) for k, v in sampling_to_log.items() if isinstance(v, (int, float, torch.Tensor))}
        scalar_sampling["test/generation_time_s"] = test_gen_time          # sampling only (set above, before metrics)
        scalar_sampling["test/n_generated"] = float(n_generated)
        scalar_sampling["test/per_graph_generation_time_s"] = per_graph_s
        self.log_dict(scalar_sampling, sync_dist=True)
        print(f"Done. Sampling took {test_gen_time:.2f} seconds (excluding metric computation)\n")
        print(f"Test ends.")
    ...
    def sample_batch(
        ...
        for s_int in reversed(range(0, self.T, self.cfg.general.faster_sampling)):
            ...
            z_s = self.sample_zs_from_zt(z_t=z_t, s_int=s_array, mu=mu)
    ...
    def sample_zs_from_zt(self, z_t, s_int, mu=None):
    ...
    def sample_n_graphs(
        ...
        progress = (_ProgressTracker(total=samples_to_generate, label=progress_label)
        ...
        if first_sampling > 0:
            ...
            for i, n in enumerate(n_nodes):
                ...
                if potential_ebs > len(current_n_list) or len(current_n_list) == 0:
                    ...
                    if progress: progress.update(len(current_n_list))
