...
def main(cfg: DictConfig):
    ...
    if dataset_config.name in [
        ...
        dataset_infos = TLSInfos(datamodule, cfg)
    else:
    ...
    if not hasattr(cfg.model, "is_baseline"):
        # Re-seed here so model weight init is deterministic regardless of
        # RNG consumed during dataset/ref-metrics setup above.
        pl.seed_everything(cfg.train.seed)
