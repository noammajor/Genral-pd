...
class XEyTransformerLayer(nn.Module):
    ...
    def forward(self, features: utils.PlaceHolder, H_G0=None):
        """Pass the input through the encoder layer.
        X: (bs, n, d)
        E: (bs, n, n, d)
        y: (bs, dy)
        node_mask: (bs, n) Mask for the src keys per batch (optional)
        H_G0: (bs, n_head, df) optional TAM bias — broadcast over n×n pairs
        Output: newX, newE, new_y with the same shape.
        """
        X, E, y, node_mask = features.X, features.E, features.y, features.node_mask

        newX, newE, new_y = self.self_attn(X, E, y, node_mask=node_mask, H_G0=H_G0)
...
class NodeEdgeBlock(nn.Module):
    ...
    def forward(self, X, E, y, node_mask, H_G0=None):
        """
        :param X: bs, n, d        node features
        :param E: bs, n, n, d     edge features
        :param y: bs, dz           global features
        :param node_mask: bs, n
        :param H_G0: (bs, n_head, df) optional TAM topology bias
        ...
        # TAM: add topology bias H_G0 broadcast over all node pairs
        if H_G0 is not None:
            Y = Y + H_G0.unsqueeze(1).unsqueeze(1)  # (bs,1,1,n_head,df)
...
class GraphTransformer(nn.Module):
    ...
    def __init__(
        ...
        # TAM: project µ_G₀ → (n_head, df) bias injected into every layer
        self.use_tam = mu_dim > 0
        if self.use_tam:
            self.topo_proj = nn.Linear(mu_dim, hidden_dims["n_head"] * (hidden_dims["dx"] // hidden_dims["n_head"]))

    def forward(self, model_input, mu=None):
        X = model_input.X
        E = model_input.E
        y = model_input.y
        ...
        # TAM: project µ → H_G0 bias applied at every layer
        H_G0 = None
        if self.use_tam and mu is not None:
            H_G0 = self.topo_proj(mu)                           # (bs, n_head*df)
            H_G0 = H_G0.reshape(mu.shape[0], self.tf_layers[0].self_attn.n_head,
                                 self.tf_layers[0].self_attn.df)  # (bs, n_head, df)

        for layer in self.tf_layers:
            features = layer(features, H_G0=H_G0)
